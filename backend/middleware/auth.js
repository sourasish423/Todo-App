...
const auth=(req,res,next)=>{
    ...
    try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET);//The backend then verifies the token using jwt.verify() with the same secret key that was used to sign the token during login.
