...
export default function Login() {
  ...
  const handleLogin = async() => {
    ...
    try {
      const response = await fetch("https://todo-app-production-7a23.up.railway.app/signup", {
        method: "POST",
      ...
      alert(data.message);//displays an alert with the message received from the backend

      if (response.ok) {//only navigate if login was successful
        localStorage.setItem("token", data.token);//stores the token in local storage
        navigate("/tasks");//navigates the user to the tasks page after a successful login
      }
    }
